// Build tasks for the InfraSpec CLI.
//
...
//     - INFRASPEC_VERSION: determined from source by default
...
var tasks = map[string]func(string) error{
	"bin/infraspec": func(exe string) error {
		...
		ldflags = fmt.Sprintf("-X github.com/robmorgan/infraspec/internal/build.Version=%s %s", version(), ldflags)
		ldflags = fmt.Sprintf("-X github.com/robmorgan/infraspec/internal/build.Date=%s %s", date(), ldflags)
...
func main() {
	...
	if len(args) < 2 {
		if isWindowsTarget() {
			...
			args = append(args, "bin/infraspec")
		}
	}
...
func version() string {
	if versionEnv := os.Getenv("INFRASPEC_VERSION"); versionEnv != "" {
		return versionEnv
	}
