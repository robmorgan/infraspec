...
# This configuration follows security best practices and will pass all gatekeeper checks
...
resource "aws_s3_bucket_logging" "secure_bucket" {
  ...
  target_prefix = "log/secure_bucket/"
}
...
# Secure security group - restricted ingress and egress
resource "aws_security_group" "web" {
  name        = "web-sg"
  description = "Security group for web servers"
  vpc_id      = aws_vpc.main.id

  # Allow HTTPS from specific ranges
  ingress {
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
...
# IAM role with managed policy (no inline)
...
# Default security group with no rules
resource "aws_default_security_group" "default" {
  vpc_id = aws_vpc.main.id

  # No ingress or egress rules - use explicit security groups
  tags = {
