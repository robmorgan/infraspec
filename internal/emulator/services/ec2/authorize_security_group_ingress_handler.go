...
func (s *EC2Service) authorizeSecurityGroupIngress(ctx context.Context, params map[string]interface{}) (*emulator.AWSResponse, error) {
	...
	// Parse IP permissions from params
	permissions := s.parseIpPermissions(params)
	sg.IpPermissions = append(sg.IpPermissions, permissions...)
