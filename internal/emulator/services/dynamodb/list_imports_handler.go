...
func (s *DynamoDBService) listImports(ctx context.Context, input *ListImportsInput) (*emulator.AWSResponse, error) {
	...
	for _, key := range keys {
		...
		// Add S3 bucket source if present
	...
	if input.NextToken != nil && *input.NextToken != "" {
		...
		// For simplicity, we'll find the matching import ARN
		for i, summary := range importSummaries {
			if summary.ImportArn != nil && *summary.ImportArn == *input.NextToken {
				startIndex = i + 1
				break
			}
		}
	...
	if endIndex < len(importSummaries) {
		if nextSummary := importSummaries[endIndex]; nextSummary.ImportArn != nil {
			response["NextToken"] = *nextSummary.ImportArn
		}
	}
