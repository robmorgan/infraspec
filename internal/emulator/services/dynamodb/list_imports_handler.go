...
func (s *DynamoDBService) listImports(ctx context.Context, input *ListImportsInput) (*emulator.AWSResponse, error) {
	...
	if input.NextToken != nil && *input.NextToken != "" {
		// NextToken format: "index:<number>"
		if strings.HasPrefix(*input.NextToken, "index:") {
			var idx int
			if _, err := fmt.Sscanf(*input.NextToken, "index:%d", &idx); err == nil {
				startIndex = idx
			}
		}
	}
	...
	if endIndex < len(importSummaries) {
		response["NextToken"] = fmt.Sprintf("index:%d", endIndex)
	}
