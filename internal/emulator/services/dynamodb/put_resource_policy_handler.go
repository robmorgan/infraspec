...
// putResourcePolicy attaches a resource-based policy document to a table or stream.
// When a policy is attached, the policy application is eventually consistent.
func (s *DynamoDBService) putResourcePolicy(ctx context.Context, input *PutResourcePolicyInput) (*emulator.AWSResponse, error) {
	...
	// State key for resource policy (keyed by full ARN, consistent with GetResourcePolicy)
	policyKey := fmt.Sprintf("dynamodb:resource-policy:%s", resourceArn)

	// Check expected revision ID if provided
