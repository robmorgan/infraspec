...
func TestListGlobalTables_WithGlobalTables(t *testing.T) {
	...
	table1Key := "dynamodb:globaltable:global-table-1"
	...
	table2Key := "dynamodb:globaltable:global-table-2"
...
func TestListGlobalTables_FilterByRegion(t *testing.T) {
	...
	tables, ok := responseBody["GlobalTables"].([]interface{})
	require.True(t, ok, "GlobalTables should be an array")
	assert.Len(t, tables, 1, "Should have only one global table in us-east-1")

	tableMap, ok := tables[0].(map[string]interface{})
	require.True(t, ok)
	assert.Equal(t, "global-table-1", tableMap["GlobalTableName"])
}
...
func TestListGlobalTables_Pagination(t *testing.T) {
	...
	for i := 1; i <= 5; i++ {
		tableKey := fmt.Sprintf("dynamodb:globaltable:global-table-%d", i)
