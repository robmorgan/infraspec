...
func TestListGlobalTables_WithGlobalTables(t *testing.T) {
	...
	table1Key := fmt.Sprintf("dynamodb:globaltable:%s", table1Name)
	...
	table2Key := fmt.Sprintf("dynamodb:globaltable:%s", table2Name)
...
func TestListGlobalTables_FilterByRegionName(t *testing.T) {
	state := emulator.NewMemoryStateManager()
	validator := emulator.NewSchemaValidator()
	service := NewDynamoDBService(state, validator)

	// Create global tables with replicas in different regions
	table1Name := "global-table-1"
	table1Key := fmt.Sprintf("dynamodb:globaltable:%s", table1Name)
	table1Data := map[string]interface{}{
		"GlobalTableName": table1Name,
	...
	// List global tables with replicas in us-east-1
	regionName := "us-east-1"
	input := &ListGlobalTablesInput{
	...
	tables, ok := responseBody["GlobalTables"].([]interface{})
	require.True(t, ok, "GlobalTables should be an array")
	assert.Len(t, tables, 1, "Should have only one global table with replica in us-east-1")
