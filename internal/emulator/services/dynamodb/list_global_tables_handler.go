...
func (s *DynamoDBService) listGlobalTables(ctx context.Context, input *ListGlobalTablesInput) (*emulator.AWSResponse, error) {
	...
	for _, key := range keys {
		...
		if tableName, ok := globalTableData["GlobalTableName"].(string); ok {
			globalTable.GlobalTableName = &tableName
		}
	...
	if input.ExclusiveStartGlobalTableName != nil && *input.ExclusiveStartGlobalTableName != "" {
		// Find the index of the exclusive start global table name
	...
	if endIndex < len(globalTables) {
		if nextTable := globalTables[endIndex]; nextTable.GlobalTableName != nil {
			response["LastEvaluatedGlobalTableName"] = *nextTable.GlobalTableName
		}
	}
