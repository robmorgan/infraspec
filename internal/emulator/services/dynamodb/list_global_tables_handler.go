...
func (s *DynamoDBService) listGlobalTables(ctx context.Context, input *ListGlobalTablesInput) (*emulator.AWSResponse, error) {
	...
	paginatedGlobalTables := []GlobalTable{}
	...
	if endIndex < len(globalTables) {
		if len(paginatedGlobalTables) > 0 {
			lastGT := paginatedGlobalTables[len(paginatedGlobalTables)-1]
			if lastGT.GlobalTableName != nil {
				response.LastEvaluatedGlobalTableName = lastGT.GlobalTableName
			}
		}
	}
