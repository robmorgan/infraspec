...
func TestListContributorInsights_FilterByTableName(t *testing.T) {
	...
	table1Name := "table1"
	insight1Key := fmt.Sprintf("dynamodb:contributorinsights:%s", table1Name)
	insight1Data := map[string]interface{}{
	...
	table2Name := "table2"
	insight2Key := fmt.Sprintf("dynamodb:contributorinsights:%s", table2Name)
	insight2Data := map[string]interface{}{
...
func TestListContributorInsights_Pagination(t *testing.T) {
	...
	for i := 1; i <= 5; i++ {
		insightKey := fmt.Sprintf("dynamodb:contributorinsights:%s:index%d", tableName, i)
	...
	// List contributor insights with max results
	maxResults := int32(2)
	input := &ListContributorInsightsInput{
	...
	summaries, ok := responseBody["ContributorInsightsSummaryList"].([]interface{})
	require.True(t, ok, "ContributorInsightsSummaryList should be an array")
	assert.Len(t, summaries, 2, "Should have only 2 contributor insights due to max results")
