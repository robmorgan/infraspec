...
// immediately. If it returns any other type of error, sleep for sleepBetweenRetries and try again, up to a maximum of
// maxRetries retries. If maxRetries is exceeded, return a MaxRetriesExceeded error.
func DoWithRetryInterface(actionDescription string, maxRetries int, sleepBetweenRetries time.Duration, action func() (interface{}, error)) (interface{}, error) {
	var output interface{}
	var err error

	for i := 0; i <= maxRetries; i++ {
		config.Logging.Logger.Infof("%s", actionDescription)
		...
		config.Logging.Logger.Infof("%s returned an error: %s. Sleeping for %s and will try again.", actionDescription, err.Error(), sleepBetweenRetries)
	...
	return DoWithRetry(actionDescription, maxRetries, sleepBetweenRetries, func() (string, error) {
		...
		for errorRegexp, errorMessage := range retryableErrorsRegexp {
			if errorRegexp.MatchString(output) || errorRegexp.MatchString(err.Error()) {
				config.Logging.Logger.Infof("'%s' failed with the error '%s' but this error was expected and warrants a retry. Further details: %s\n", actionDescription, err.Error(), errorMessage)
				return output, err
			}
		}
...
func DoInBackgroundUntilStopped(actionDescription string, sleepBetweenRepeats time.Duration, action func()) Done {
	stop := make(chan bool)

	go func() {
		for {
			config.Logging.Logger.Infof("Executing action '%s'", actionDescription)

			action()

			config.Logging.Logger.Infof("Sleeping for %s before repeating action '%s'", sleepBetweenRepeats, actionDescription)

			select {
			case <-time.After(sleepBetweenRepeats):
				// Nothing to do, just allow the loop to continue
			case <-stop:
				config.Logging.Logger.Infof("Received stop signal for action '%s'.", actionDescription)
				return
			}
		}
	}()
