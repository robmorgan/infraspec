...
func TestDoWithRetry(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.description, func(t *testing.T) {
			...
			actualOutput, err := DoWithRetry(testCase.description, testCase.maxRetries, 1*time.Millisecond, testCase.action)
...
func TestDoInBackgroundUntilStopped(t *testing.T) {
	...
	stop := DoInBackgroundUntilStopped(t.Name(), sleepBetweenRetries, func() {
		counter++
...
func TestDoWithRetryableErrors(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.description, func(t *testing.T) {
			...
			actualOutput, err := DoWithRetryableErrors(testCase.description, testCase.retryableErrors, testCase.maxRetries, 1*time.Millisecond, testCase.action)
			assert.Equal(t, expectedOutput, actualOutput)
			if testCase.expectedError != nil {
				assert.Equal(t, testCase.expectedError, err)
			} else {
