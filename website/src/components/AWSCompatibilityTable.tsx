...
function CoverageBar({ percent }: { percent: number }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-sm text-gray-600 dark:text-gray-400 min-w-[3rem] text-right">{percent.toFixed(1)}%</span>
    </div>
  );
}
...
function ServiceRow({ service }: { service: Service }) {
  ...
  const hasDetails =
    (service.infraspec?.operations?.length ?? 0) > 0 || (service.virtualCloud?.operations?.length ?? 0) > 0;

  return (
    <div className="border border-gray-300 dark:border-gray-700 rounded-lg overflow-hidden bg-white dark:bg-gray-900">
      <button
        onClick={() => hasDetails && setIsExpanded(!isExpanded)}
        className={`w-full px-4 py-3 flex items-center justify-between text-left bg-gray-50 dark:bg-gray-900 ${
          hasDetails ? "hover:bg-gray-100 dark:hover:bg-gray-800/50 cursor-pointer" : ""
        }`}
        disabled={!hasDetails}
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-semibold text-gray-800 dark:text-gray-100">{service.fullName}</div>
            <div className="text-sm font-medium text-gray-600 dark:text-gray-400">{service.name.toUpperCase()}</div>
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300">InfraSpec Assertions</h4>
                {service.infraspec && <StatusBadge status={service.infraspec.status} />}
              </div>
              {service.infraspec?.operations && service.infraspec.operations.length > 0 ? (
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 italic">No assertions available yet</p>
              )}
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Virtual Cloud API</h4>
                {service.virtualCloud && <StatusBadge status={service.virtualCloud.status} />}
              </div>
              {service.virtualCloud ? (
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 italic">Not available in Virtual Cloud</p>
              )}
            </div>
          </div>
        </div>
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-green-50 dark:bg-green-900/20 rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-green-700 dark:text-green-400">{implementedServices.length}</div>
          <div className="text-sm text-green-600 dark:text-green-500">Implemented</div>
        </div>
        <div className="bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-yellow-700 dark:text-yellow-400">{plannedServices.length}</div>
          <div className="text-sm text-yellow-600 dark:text-yellow-500">Planned</div>
        </div>
        <div className="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-purple-700 dark:text-purple-400">
            {implementedServices.reduce((acc, s) => acc + (s.infraspec?.operations?.length || 0), 0)}
          </div>
          <div className="text-sm text-purple-600 dark:text-purple-500">InfraSpec Assertions</div>
        </div>
        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-blue-700 dark:text-blue-400">
            {implementedServices.reduce((acc, s) => acc + (s.virtualCloud?.implemented || 0), 0)}
          </div>
          <div className="text-sm text-blue-600 dark:text-blue-500">Virtual Cloud APIs</div>
        </div>
      </div>
      ...
      {implementedServices.length > 0 && (
        <div>
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Implemented Services</h3>
          <div className="space-y-3">
      ...
      {plannedServices.length > 0 && (
        <div>
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Planned Services</h3>
          <div className="space-y-3">
