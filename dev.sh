...
#   cd ../infraspec-api && go run ./cmd/emulator --port 8000 --auth-enabled --api-keys "infraspec-api:securetoken"
#
# Environment variables:
#   INFRASPEC_LOCAL_API_URL  - Local API URL (default: http://localhost:8000)

set -euo pipefail

LOCAL_API_URL="${INFRASPEC_LOCAL_API_URL:-http://localhost:8000}"
...
if ! curl -s --connect-timeout 2 "$LOCAL_API_URL/_health" > /dev/null 2>&1; then
    ...
    echo ""
    echo "Start it with:"
    echo "  cd ../infraspec-api && go run ./cmd/emulator --port 8000 --auth-enabled --api-keys \"infraspec-api:securetoken\""
    echo ""
